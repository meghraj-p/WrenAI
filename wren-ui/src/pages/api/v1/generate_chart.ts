...
const validateChartResult = (result: ChartResult): void => {
  ...
  if (result.status === ChartStatus.FAILED || result.error) {
    throw new ApiError(
      result.error?.message || 'Failed to generate chart',
      400,
      Errors.GeneralErrorCodes.FAILED_TO_GENERATE_CHART,
    );
  }

  // Verify that the chartSchema is present
  if (!result?.response?.chartSchema) {
    throw new ApiError('Failed to generate chart', 500);
  }
};
...
export default async function handler(
  ...
  const {
    ...
    threadId,
    sampleSize = 10000,
  } = req.body as GenerateChartRequest;
  ...
  try {
    ...
    // Ask AI service to generate a chart
    ...
    if (!task || !task.queryId) {
      throw new ApiError('Failed to start chart generation task', 500);
    }
    ...
    while (true) {
      ...
      if (Date.now() > deadline) {
        throw new ApiError(
          'Timeout waiting for chart generation',
          500,
    ...
    // Get the generated chart spec
    const chartSpec = result?.response?.chartSchema;

    // Return the chart spec with data included
    await respondWith({
      res,
      statusCode: 200,
      responsePayload: {
      ...
      apiType: ApiType.GENERATE_CHART,
      startTime,
